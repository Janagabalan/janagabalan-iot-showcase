...
import { Code, Cpu, Cloud, Settings, FileText, BarChart3 } from "lucide-react";
    ...
    <section id="skills" className="py-20 px-4 bg-slate-50 dark:bg-slate-900/50">
      <div className="container mx-auto">
                ...
                {category.skills.map((skill, skillIndex) => (
                  <div key={skillIndex} className="space-y-2">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-teal-600 dark:text-teal-400 text-sm font-semibold">{skill.level}%</span>
                    </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <div className="text-center bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg border border-slate-200 dark:border-slate-700">
            <div className="text-3xl font-bold text-teal-600 dark:text-teal-400 mb-2">5+</div>
            <div className="text-slate-600 dark:text-slate-400 font-medium">IoT Projects</div>
          </div>
          <div className="text-center bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg border border-slate-200 dark:border-slate-700">
            <div className="text-3xl font-bold text-teal-600 dark:text-teal-400 mb-2">2</div>
            <div className="text-slate-600 dark:text-slate-400 font-medium">Internships</div>
          </div>
          <div className="text-center bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg border border-slate-200 dark:border-slate-700">
            <div className="text-3xl font-bold text-teal-600 dark:text-teal-400 mb-2">3+</div>
            <div className="text-slate-600 dark:text-slate-400 font-medium">Cloud Platforms</div>
          </div>
          <div className="text-center bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg border border-slate-200 dark:border-slate-700">
            <div className="text-3xl font-bold text-teal-600 dark:text-teal-400 mb-2">4</div>
            <div className="text-slate-600 dark:text-slate-400 font-medium">Years Study</div>
          </div>
        </div>
      </div>
    </section>
