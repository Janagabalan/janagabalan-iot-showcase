  ...
  return (
    <section id="about" className="py-20 px-4">
      <div className="container mx-auto">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-lg border border-slate-200 dark:border-slate-700">
              ...
              <div className="space-y-4">
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-slate-600 dark:text-slate-400">Tittakudi, Cuddalore District</span>
                </div>
                <div className="flex items-center space-x-3">
                  <GraduationCap className="h-5 w-5 text-teal-600 dark:text-teal-400" />
                  <span className="text-slate-600 dark:text-slate-400">ECE Student (Final Year)</span>
                </div>
                <div className="flex items-center space-x-3">
                  <Calendar className="h-5 w-5 text-teal-600 dark:text-teal-400" />
                  <span className="text-slate-600 dark:text-slate-400">Expected Graduation: 2026</span>
                </div>
              </div>
            </div>

            <div className="bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-lg border border-slate-200 dark:border-slate-700">
              <h3 className="text-2xl font-bold text-slate-800 dark:text-white mb-4">My Passion</h3>
              ...
              <div key={index} className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg border border-slate-200 dark:border-slate-700">
                <div className="flex items-start space-x-4">
                  ...
                  <div className="flex-grow">
                    <h4 className="text-lg font-semibold text-slate-800 dark:text-white">{edu.degree}</h4>
                    <p className="text-teal-600 dark:text-teal-400 font-medium">{edu.institution}</p>
                    <p className="text-slate-500 dark:text-slate-400 text-sm">{edu.location} • {edu.period}</p>
                    <span className="inline-block mt-2 px-3 py-1 bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300 text-xs rounded-full">
                      {edu.status}
                    </span>
                  </div>
                </div>
        ...
        <div>
          <h3 className="text-2xl font-bold text-slate-800 dark:text-white mb-8 text-center">Internship Experience</h3>
          <div className="grid md:grid-cols-2 gap-8">
            {internships.map((internship, index) => (
              <div key={index} className="bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-lg border border-slate-200 dark:border-slate-700 hover:shadow-xl transition-shadow duration-300">
                <div className="flex items-center space-x-3 mb-4">
                  <Award className="h-6 w-6 text-teal-600 dark:text-teal-400" />
                  <h4 className="text-xl font-semibold text-slate-800 dark:text-white">{internship.company}</h4>
                </div>
                <p className="text-teal-600 dark:text-teal-400 font-medium mb-2">{internship.role}</p>
                <p className="text-slate-500 dark:text-slate-400 text-sm mb-3">{internship.duration} • {internship.date}</p>
                <p className="text-slate-600 dark:text-slate-400 leading-relaxed">{internship.description}</p>
              </div>
            ))}
          </div>
        </div>
